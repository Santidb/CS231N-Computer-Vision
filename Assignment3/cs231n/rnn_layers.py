...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    # Single forward step is the tanh( h*Wh + x*Wx + b )
    h_mat = np.dot(prev_h, Wh)
    x_mat = np.dot(x, Wx)
    sum_all = h_mat + x_mat + b
    next_h = np.tanh(sum_all)

    # Storing results in cache
    cache = (x, prev_h, Wx, Wh, b, sum_all)
...
def rnn_step_backward(dnext_h, cache):
    ...
    x, prev_h, Wx, Wh, b, sum_all = cache

    # Backprop sum_all
    dsum_all = (1-(np.tanh(sum_all))**2) * dnext_h

    # Backprop X, Wx
    dx = np.dot(dsum_all, Wx.T)
    dWx = np.dot(x.T, dsum_all)

    # Backprop h, Wh
    dprev_h = np.dot(dsum_all, Wh.T)
    dWh = np.dot(prev_h.T, dsum_all)

    # Backprop b
    db = np.sum(dsum_all, axis=0)

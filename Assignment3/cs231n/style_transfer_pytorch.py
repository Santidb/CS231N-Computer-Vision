...
def content_loss(content_weight, content_current, content_original):
    ...
    sum_all = torch.sum((content_current - content_original)**2)
    loss = sum_all * content_weight
    return loss
...
def gram_matrix(features, normalize=True):
    ...
    # Flattening dimensions of features
    features_flat = features.view(N, C, H*W)

    # Initializing gram
    gram = torch.zeros(N, C, C)
    
    # Performing sum over every element of gram, for every image
    for n in range(N):
      gram[n, :, :] = features_flat[n, :, :].mm(torch.transpose(features_flat[n,:,:], 1, 0))
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    # Looping over every layer
    for l in range(layers):
      
      # Obtaining source gram
      source_gram = style_targets[l]
      
      # Obtaining current image gram
      idx = style_layers[l]
      img_gram = gram_matrix(feats[idx], normalize=True)

      # Extracting weight
      weight = style_weights[l]
      
      # Computing loss at this layer
      loss += weight * torch.sum((img_gram - source_gram)**2)
...
def tv_loss(img, tv_weight):
    ...
    # Getting img matrix without first row and without last row
    img_i_end = img[:, :, 1:, :]
    img_i_start = img[:, :, :-1, :]

    # Getting img matrix without first column and without last column
    img_j_end = img[:, :, :, 1:]
    img_j_start = img[:, :, :, :-1]

    # Combining it to get the loss
    loss = tv_weight * (torch.sum((img_i_end - img_i_start)**2) + torch.sum((img_j_end - img_j_start)**2))
